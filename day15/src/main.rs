...
use iced::{
    canvas::{self, Cursor, Frame, Path, Stroke},
    executor,
    futures::stream::Collect,
    time, window, Application, Canvas, Color, Command, Element, Length, Point, Rectangle, Settings,
...
impl<Message> canvas::Program<Message> for Table {
    fn draw(&self, bounds: Rectangle, cursor: Cursor) -> Vec<canvas::Geometry> {
        let mut frame = Frame::new(bounds.size());
        let circle = Path::circle(frame.center(), 5.0);
        let line = Path::line(
            Point { x: 10.0, y: 10.0 },
            Point {
                x: frame.width(),
